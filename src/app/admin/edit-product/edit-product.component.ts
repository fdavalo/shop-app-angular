...
export class EditProductComponent implements OnInit, AfterViewChecked {
  product = {id:0,name:'',price:0,display:'',waterResistant:'',camera:'',frontCamera:'',futures:'',processor:'',charging:'',memory:[],image:[]};
  id: number;
  selectedMemory = [];
  img: string;
  ...
  postJson(data) {
    if ((data.name != this.product.name) || (data.price != this.product.price)) {
      this.product.name = data.name;
      this.product.price = data.price;
      this.cartService.updateProduct(this.product, this.product.id).subscribe(() => {
      });
    }
  }
