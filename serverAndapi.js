...
server.use((req, res, next) => {
  if (staticPath(req)) {
    var dir = __dirname;
    if (req.path == '/') res.sendFile(path.join(dir + '/dist/shop-app-angular/index.html'));
    else res.sendFile(path.join(dir + '/dist/shop-app-angular'+req.path));
  }
  else {
	next();
  }
...
function staticPath(req) {
	...
	if (req.path.startsWith('/orders')) return false;
	if (req.path.startsWith('/sessions')) return false;
	if (req.path.startsWith('/products')) return false;
